...
const API_BASE_URL = "http://localhost:3000/api"; // To Be uncommented at home
/* const API_BASE_URL =
  "https://friendly-dollop-7v54wq95prjxfpxvv-3000.app.github.dev/api"; // To be commented at home
 */
...
export async function fetchDepartments() {
  try {
    ...
    if (online) {
      const response = await fetch(`${API_BASE_URL}/departments`);
      if (!response.ok) throw new Error("fail to get departments");
      const data = await response.json();
      await AsyncStorage.setItem("people", JSON.stringify(data));
      return data;
    } else {
      const cacheData = await AsyncStorage.getItem("departments");
      if (cacheData) {
